...
import { PublishedCollection, getPopularPublishedCollections } from '../db';
import ExploreCollectionCard from '../components/exploreCollectionCard';

export default function PopularExploreList() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getPopularPublishedCollections(200);
        setItems(data);
      } catch (e) {
        console.error(e);
      }
  ...
  return (
    <View style={styles.container}>
      <Text style={{ ...styles.text, fontWeight: 700, marginHorizontal: 16, marginBottom: 8 }}>Popular</Text>
      <FlatList
        data={items}
